...
const CalendarData = ({ onDateChange }) => {
  ...
  return (
    <div className="relative w-full max-w-full">
      ...
      >
        <h1 className="text-lg font-semibold md:mb-0 md:text-xl">
          Student Attendance
        </h1>
      ...
      {isCalendarOpen && (
        <div className="absolute top-full right-0 mt-1 w-full max-w-xs md:max-w-md lg:max-w-lg z-10 border border-gray-300 rounded-lg shadow-lg bg-white">
          <Calendar
            ...
            value={date}
            tileClassName={({ date, view }) => (view === 'month' && date.toDateString() === new Date().toDateString() ? 'bg-blue-500 text-white rounded-full' : '')}
            className="w-full"
          />
        </div>
      )}
